  ...
  async availability(
    ...
    for (let date = new Date(startDate); date <= lastDate; date.setDate(date.getDate() + 1)) {
      ...
      ( 
        ...
        .find({ where: [
          {product: {id: product.id}, startDate: LessThanOrEqual(startOfDate), endDate: MoreThanOrEqual(startOfDate)},
          {product: {id: product.id}, startDate: MoreThanOrEqual(startOfDate), endDate: MoreThanOrEqual(endOfDate)},
          {product: {id: product.id}, startDate: LessThanOrEqual(startOfDate), endDate: MoreThanOrEqual(endOfDate)}
        ]}) 
      );
